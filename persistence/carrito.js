class Carrito {
    ...
    async read() {
        ...
        try {
            const content= await this.fs.promises.readFile(this.fileName,'utf-8');
    ...
    async modify(producto){
        ...
        while (i<content.length && found  == 0){
            if (content[i].id == producto.id){
                ...
                content[i] = producto;
                try {    
                    await this.fs.promises.writeFile(this.fileName,JSON.stringify(content)); 
                    return producto;
                } catch (e) {
                    return [];
                }
    ...
    async delete() {
        try {
            await this.fs.promises.unlink(this.fileName);
        } catch (error) {
